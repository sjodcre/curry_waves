        ...
        reader.onload = async (event) => {
          ...
          if (data) {
            ...
            try {
                ...
                if (processSrc) {
                    processSrc = processSrc.replace(new RegExp('<CREATOR>', 'g'), arProvider?.profile?.id || "ANON");
                    processSrc = processSrc.replace(new RegExp(`'<NAME>'`, 'g'), cleanProcessField(postTitle));
                    processSrc = processSrc.replace(new RegExp('<TICKER>', 'g'), 'ATOMIC');
                    processSrc = processSrc.replace(new RegExp('<DENOMINATION>', 'g'), '1');
                    processSrc = processSrc.replace(new RegExp('<BALANCE>', 'g'), balance.toString());
                    processSrc = processSrc.replace(new RegExp('<COLLECTION>', 'g'), "");
                }
